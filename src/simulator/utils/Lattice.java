...
import algorithms.SSSP.Dijkstra.DijkstraCore;
import algorithms.SSSP.SPFA.SPFACore;
...
public class Lattice extends ProcessorArchitecture {
    ...
    public Core getNorthernNeighbor(int row, int col) {
        row = (row + N_ROW - 1) % N_ROW;
        return getCore(getID(row, col));
    }
    public Core getSouthernNeighbor(int row, int col) {
        row = (row + 1) % N_ROW;
        return getCore(getID(row, col));
    }

    public Core getEasternNeighbor(int row, int col) {
        col = (col + 1) % N_ROW;
        return getCore(getID(row, col));
    }
    public Core getWesternNeighbor(int row, int col) {
